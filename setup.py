...
setup  ={'1':{'vel':0.002,'steps':50000,'dcd':500,'howmany':99,'tclfreq':50},
      '2':{'vel':0.0002,'steps':500000,'dcd':5000,'howmany':48,'tclfreq':50},
      '3':{'vel':0.00002,'steps':5000000,'dcd':50000,'howmany':30,
                                                               'tclfreq':50},
      '4':{'vel':0.000002,'steps':50000000,'dcd':500000,'howmany':3,
                                                               'tclfreq':50},
      '5':{'vel':0.0000002,'steps':500000000,'dcd':5000000,'howmany':1,
                                                               'tclfreq':50}}
...
def re_job(script,mol,env,v):
    o=open(script,'r+')
    text=o.read()
    bashjobname=mol+'.namd.'+str((setup[v]['vel'])*500000)+env.split('.')[1]
...
def re_expavg(script,mol,env,v):
    ...
    plotn1=mol+'namd'+env.split('.')[1]+str(confige[v])
    ...
    dt=str(((setup[v]['tclfreq'])*(tstep[ts])/1000))
...
def reg_exp(subdir,mol,env,v):        # regular expression by script name
...
for mol in molec:
    ...
    tops=os.path.join(workdir,'toppar')
    topd=os.path.join(jobdir,'toppar')
    shutil.copytree(tops,topd)
#________________________________________________________________________
confd={}                                                       # a pickle
confd['mlist']=mlist
confd['molec']=molec
